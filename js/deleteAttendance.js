...
import { getFirestore, collection, getDocs, deleteField, updateDoc, deleteDoc } from 'https://www.gstatic.com/firebasejs/10.12.4/firebase-firestore.js';
...
del.addEventListener("click", function(e){
    ...
    async function del1() {
        ...
        docSnap.forEach((doc) => {
            updateDoc(doc.ref, {
                [fieldNameToDelete]: deleteField()
            }).then(() => {
            }).catch((error) => {
                console.error(`Error deleting field from document: ${doc.id}`, error);
            });
        });
        const querySnap = await getDocs(causeCollection);
        let found = false;
        ...
        setTimeout(function() {
            if (found) {
                alert("Attendance of "+date1+" is deleted.");
            } else{
                alert("No attendance found for the date: " + date1);
            }
